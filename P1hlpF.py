...
def process_image(image, iDir='',  iFnm='', saveFs=0):
    ...
    Verts    = namedtuple('Verts', ['lT', 'lB', 'rT', 'rB'])
    Top = 330
    lV  = Verts( (0,imshape[0]),(450, Top), (500, Top), (500,imshape[0]) )
    rV  = Verts( (500,imshape[0]),(500, Top), (550, Top), (imshape[1],imshape[0]) )
    lm_edges  = region_of_interest(edges, vertices = np.array([[lV.lT, lV.lB, lV.rT, lV.rB ]], dtype=np.int32))
    rm_edges  = region_of_interest(edges, vertices = np.array([[rV.lT, rV.lB, rV.rT, rV.rB ]], dtype=np.int32))
